...
> Источник: [https://dev.vk.ru/ru/vkmaps/maps-mobile-sdk/ios](https://dev.vk.ru/ru/vkmaps/maps-mobile-sdk/ios)
**MapsSDK** - библиотека, позволяющая использовать картографические данные и геосервисы VK в ваших iOS-приложениях.

:::note
Для использования MapsSDK вам необходим API-ключ. Подробнее о том, как его получить вы можете прочитать [`по этой ссылке`](vkmaps/general-information/api-key)
:::
...
* в появившемся окне введите адрес репозитория - https://github.com/maps-mailru/maps-sdk-ios.
...
* [`API_KEY`](vkmaps/general-information/api-key) для работы с SDK;
...
Поддерживается отрисовка маршрутов в формате кодированной строки из сервиса [Построение маршрута](vkmaps/routing/direction).
...
`Geocoder` — компонент `MapsSDK` для прямого и обратного геокодирования. Для инициализации необходим [`API_KEY`](vkmaps/general-information/api-key).
