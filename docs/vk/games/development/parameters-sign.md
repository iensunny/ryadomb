...
> Источник: [https://dev.vk.ru/ru/games/development/parameters-sign](https://dev.vk.ru/ru/games/development/parameters-sign)
Игры и мини-приложения ВКонтакте используют подпись [параметров](games/development/parameters), чтобы защититься от подмены значений и возможной утечки данных.

1. При запуске приложения ВКонтакте вычисляет подпись с помощью [защищённого ключа доступа](games/settings/development/keys#Защищённый%20ключ) из настроек. Этот ключ известен авторам игры или мини-приложения, но неизвестен третьим лицам.

    Полученное значение передаётся приложению в [параметре запуска `sign`](games/development/parameters).
...
1. Для получившейся строки вычислите хеш-код, используя алгоритм SHA256 с методом HMAС. При расчётах используйте защищённый ключ из настроек вашей игры (раздел [Разработка&nbsp;&rarr; Ключи доступа](games/settings/development/keys), поле **Защищённый ключ**).
