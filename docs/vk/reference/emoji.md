...
> Источник: [https://dev.vk.ru/ru/reference/emoji](https://dev.vk.ru/ru/reference/emoji)
Emoji символы содержатся в тексте сообщения в виде специальных символов. Если в системе, в которой будут отображаться эти символы, нет их поддержки, необходимо отображать символы, используя любой доступный набор иконок emoji. 
При этом все сообщения, содержащие символы, которые необходимо заменять, содержат специальное поле `emoji`; если данное поле не пришло — сообщение можно не обрабатывать. 

Если в вашем случае достаточно иконок размером 16x16px — вы можете получить их изображение по адресу: 

```
http://vk.com/images/emoji/{hex-код символа}.png 
```

### Сейчас отображаются символы со следующими кодами:
