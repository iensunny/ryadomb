# Интеграция | Публикации | Загрузка файлов | Фотография в личном сообщении

> Источник: [https://dev.vk.ru/ru/api/upload/photo-in-message](https://dev.vk.ru/ru/api/upload/photo-in-message)
<!-- ---
title: 'Интеграция | Публикации | Загрузка файлов | Фотография в личном сообщении'
is_hidden: false
...
Чтобы получить адрес для загрузки фотографии, вызовите метод [`photos.getMessagesUploadServer`](method/photos.getMessagesUploadServer). Если вы используете Standalone-приложение и хотите загрузить фотографию в сообщения сообщества, передайте идентификатор сообщества в параметр `group_id`.

### Запрос

```bash
curl -X POST 'https://api.vk.ru/method/photos.getMessagesUploadServer' \
...
```JSON
{
  "response":{
    "album_id":-64,
    "upload_url":"https:\/\/pu.vk.ru\/c231331\/ss2269\/upload.php?act=do_add&mid=743784474&aid=-64&gid=218252175&hash=2e60ee02a7dd5aaaf0bce3e034b1dc30&rhash=eda561bac91afb0270fc9ca741ac626b&swfupload=1&api=1&mailphoto=1",
    "user_id":0,
    "group_id":218252175
  }
}
...
```bash
curl -X POST 'https://pu.vk.ru/c231331/ss2269/upload.php?act=do_add&mid=743784474&aid=-64&gid=218252175&hash=2e60ee02a7dd5aaaf0bce3e034b1dc30&rhash=eda561bac91afb0270fc9ca741ac626b&swfupload=1&api=1&mailphoto=1' \
...
Чтобы сохранить фотографию в личном сообщении, вызовите метод [`photos.saveMessagesPhoto`](method/photos.saveMessagesPhoto) с параметрами, полученными на предыдущем этапе.
...
```bash
curl -X POST 'https://api.vk.ru/method/photos.saveMessagesPhoto' \
  -H 'Authorization: Bearer <КЛЮЧ_ДОСТУПА>' \
...
Чтобы прикрепить фотографию к личному сообщению, вызовите метод [`messages.send`](method/messages.send). В поле `attachment` укажите идентификатор фотографии в формате `photo{owner_id}_{photo_id}`, где:

* `owner_id` — поле `owner_id` из ответа метода [`photos.saveMessagesPhoto`](method/photos.saveMessagesPhoto).
* `photo_id` — поле `id` из ответа метода [`photos.saveMessagesPhoto`](method/photos.saveMessagesPhoto).
